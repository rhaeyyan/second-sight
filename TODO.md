...
- [x] ~~Replace bare `catch { return [] }` in the API routes (e.g. `src/app/api/news/route.ts:68`) with explicit `SourceHealthStatus`~~ ⚠️ partially done 2026-08-09 — two reference implementations landed, covering both route shapes: `src/app/api/fires/route.ts` (single external source) and `src/app/api/news/route.ts` (many-feed aggregator, one `SourceHealth` per feed via `X-Source-Health`). Both are fixture-tested. 11 routes still have the bare-catch pattern and need the same treatment: `alerts`, `flights`, `drones`, `oil`, `strikes`, `crypto`, `regional-alerts`, `markets`, `polymarket`, `telegram` (all aggregator-shaped, follow the news pattern), and `ships` (no real external fetch — its catch is dead defensive code, lowest priority)
