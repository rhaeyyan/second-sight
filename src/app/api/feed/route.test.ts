...
const iodaFetch = vi.fn<() => Promise<SourceAdapterResult>>();
...
vi.mock('@/lib/events/adapters/iodaOutages', () => ({
  createIodaOutagesAdapter: (): SourceAdapter => ({
...
describe('GET /api/feed', () => {
  it('merges events from all six adapters into one newest-first list', async () => {
    ...
    iodaFetch.mockResolvedValueOnce(
      result([event({ id: 'i1', reportedAt: MOCK_NOW - 5000, source: { id: 's', name: 'IODA', sourceType: 'sensor' } })], 'ioda-outages-iran-israel')
    );

    const res = await GET(new Request('http://localhost/api/feed?conflict=iran-israel'));
    const body = (await res.json()) as IronsightEvent[];

    expect(body.map((e) => e.id)).toEqual(['n1', 'c1', 'f1', 'w1', 'u1', 'i1']); // newest reportedAt first
  });
  ...
  it('populates originalTitle/originalLanguage and translates a Hebrew news title, leaving non-news events untouched', async () => {
    ...
    iodaFetch.mockResolvedValueOnce(result([], 'ioda-outages-iran-israel'));
  ...
  it('emits X-Source-Health with exactly 6 entries in [conflicts, news, firms, weather, usgs, ioda] order', async () => {
    conflictsFetch.mockResolvedValueOnce(result([], 'google-news-conflict-iran-israel'));
  ...
  it('does not crash if one adapter fetch() rejects — the other five sources still come through', async () => {
    ...
    iodaFetch.mockResolvedValueOnce(result([event({ id: 'i1' })], 'ioda-outages-iran-israel'));

    const res = await GET(new Request('http://localhost/api/feed?conflict=iran-israel'));
    const body = (await res.json()) as IronsightEvent[];

    expect(body.map((e) => e.id).sort()).toEqual(['f1', 'i1', 'n1', 'u1', 'w1']);
    ...
    expect(health[5].status).toBe('healthy');
  });
});
