...
describe('GET /api/feed', () => {
  it('merges events from all three adapters into one newest-first list', async () => {
    ...
    expect(body.map((e) => e.id)).toEqual(['n1', 'c1', 'f1']); // newest reportedAt first
  });
  ...
  it('emits X-Source-Health with exactly 3 entries in [conflicts, news, firms] order', async () => {
    conflictsFetch.mockResolvedValueOnce(result([], 'google-news-conflict-iran-israel'));
  ...
  it('does not crash if one adapter fetch() rejects — the other two sources still come through', async () => {
    ...
    expect(body.map((e) => e.id).sort()).toEqual(['f1', 'n1']);
